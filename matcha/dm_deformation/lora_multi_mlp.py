...
def initialize_lora_multi_mlp_weights(network, std=None):
    if std is not None:
        print("[INFO] Using normal initialization for MLP with std={}".format(std))
    for m in network.modules():
        if isinstance(m, LoRAMultiLinear):
            for i in range(m.n_heads):
                if std is not None:
                    nn.init.normal_(m.weight[i], std=std)
                    for j in range(m.n_lora_layers):
                        nn.init.normal_(m.lora_weight_left[j][i], std=std)
                        nn.init.normal_(m.lora_weight_right[j][i], std=std)
                else:
                    # print("[INFO] Using Xavier initialization for MLP.")
                    gain = nn.init.calculate_gain(nonlinearity='relu')
                    nn.init.xavier_normal_(m.weight[i], gain=gain)
                    for j in range(m.n_lora_layers):
                        nn.init.xavier_normal_(m.lora_weight_left[j][i], gain=gain)
                        nn.init.xavier_normal_(m.lora_weight_right[j][i], gain=gain)
...
class LoRAMultiLinear(nn.Module):
    ...
    def __init__(
        ...
        self.weight = nn.Parameter(torch.empty((n_heads, out_features, in_features), **factory_kwargs))
        self.lora_weight_left = nn.ParameterList(
            [nn.Parameter(torch.empty((n_heads, out_features * lora_rank, cond_features), **factory_kwargs))] 
            + [
                nn.Parameter(torch.empty((n_heads, out_features * lora_rank, out_features * lora_rank), **factory_kwargs))
                for _ in range(n_lora_layers-1)
            ]
        )
        self.lora_weight_right = nn.ParameterList(
            [nn.Parameter(torch.empty((n_heads, lora_rank * in_features, cond_features), **factory_kwargs))]
            + [
                nn.Parameter(torch.empty((n_heads, lora_rank * in_features, lora_rank * in_features), **factory_kwargs))
                for _ in range(n_lora_layers-1)
            ]
        )
        if bias:
    ...
    def forward(self, input: torch.Tensor, cond:torch.Tensor) -> torch.Tensor:
        # Compute LoRA matrix depending on the condition
        lora_left = torch.bmm(
            cond,  # (n_heads, batch_size, cond_features)
            self.lora_weight_left[0].transpose(1, 2),  # (n_heads, cond_features, out_features*lora_rank,)
        )  # (n_heads, batch_size, out_features*lora_rank)
        for lora_weight_left in self.lora_weight_left[1:]:
            lora_left = torch.bmm(
                torch.relu(lora_left),  # (n_heads, batch_size, out_features*lora_rank)
                lora_weight_left.transpose(1, 2),  # (n_heads, out_features*lora_rank, out_features*lora_rank,)
            )  # (n_heads, batch_size, out_features*lora_rank)
        
        lora_right = torch.bmm(
            cond,  # (n_heads, batch_size, cond_features)
            self.lora_weight_right[0].transpose(1, 2),  # (n_heads, cond_features, lora_rank*in_features,)
        )  # (n_heads, batch_size, lora_rank*in_features)
        for lora_weight_right in self.lora_weight_right[1:]:
            lora_right = torch.bmm(
                torch.relu(lora_right),  # (n_heads, batch_size, lora_rank*in_features)
                lora_weight_right.transpose(1, 2),  # (n_heads, lora_rank*in_features, lora_rank*in_features,)
            )  # (n_heads, batch_size, lora_rank*in_features)
        
        lora_matrix = torch.matmul(
            lora_left.view(-1, self.out_features, self.lora_rank),
            lora_right.view(-1, self.lora_rank, self.in_features)
        ).view(self.n_heads, -1, self.out_features, self.in_features)  # (n_heads, batch_size, out_features, in_features)
        
        # Compute output
        output = torch.matmul(
            input.reshape(-1, 1, self.in_features), # (n_heads*batch_size, 1, in_features)
            (self.weight[:, None, :, :] + lora_matrix).reshape(-1, self.out_features, self.in_features).transpose(1, 2)  # (n_heads, in_features, out_features)
        ).reshape(self.n_heads, -1, self.out_features)  # (n_heads, batch_size, out_features)
        if self.bias is not None:
            # TODO: Should make a bias that depend on the condition
            output = output + self.bias[:, None, :]
            
        return output

    def extra_repr(self) -> str:
        return 'in_features={}, out_features={}, cond_features={}, lora_rank={}, bias={}'.format(
            self.in_features, self.out_features, self.cond_features, self.lora_rank, self.bias is not None
        )
        
        
class DeformationLoRAMultiMLP(nn.Module):
    def __init__(
        self, 
        n_heads,
        n_layer,
        ...
        """
        super(DeformationLoRAMultiMLP, self).__init__()
    ...
    def forward(self, x, cond, additional_input=None):
        ...
        for layer in self.mlp:
            if isinstance(layer, LoRAMultiLinear):
                res = layer(res, cond)
            else:
                res = layer(res)
