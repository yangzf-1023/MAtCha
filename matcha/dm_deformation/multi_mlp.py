...
def initialize_multi_mlp_weights(network, std=None):
    if std is not None:
        print("[INFO] Using normal initialization for MLP with std={}".format(std))
    for m in network.modules():
        if isinstance(m, MultiLinear):
...
class MultiLinear(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        output = torch.bmm(
            ...
            self.weight.transpose(1, 2)  # (n_heads, in_features, out_features)
        )  # (n_heads, batch_size, out_features)
        if self.bias is not None:
...
class DeformationMultiMLP(nn.Module):
    def __init__(
        self, 
        n_heads,
        n_layer,
        ...
        """
        super(DeformationMultiMLP, self).__init__()
        ...
        for i in range(n_layer-2):
            ...
            layers.append(non_linearity)
        layers.append(MultiLinear(n_heads, layer_size, output_dim))
...
# ===========================================================
# ===== Following classes are just for testing purposes =====
# ===========================================================


class MultiLinearInefficient(nn.Module):
    def __init__(self, n_heads, in_features, out_features, bias=True):
        super().__init__()
        self.n_heads = n_heads
        self.in_features = in_features
        self.out_features = out_features
        self.linears = nn.ModuleList([nn.Linear(in_features, out_features, bias) for _ in range(n_heads)])
        
    def forward(self, x):
        return torch.stack([self.linears[i](x[i]) for i in range(len(self.linears))], dim=0)


class DeformationMultiMLPInefficient(nn.Module):
    def __init__(
        self, 
        n_heads,
        n_layer,
        ...
        Raises:
            ValueError: _description_
        """
        super(DeformationMultiMLPInefficient, self).__init__()
        
        from matcha.dm_deformation.mlp import DeformationMLP
        self.n_heads = n_heads
        self.n_layer = n_layer
        self.layer_size = layer_size
        self.non_linearity = non_linearity
        ...
        self.mlps = nn.ModuleList(
            DeformationMLP(
                ...
                data_input_range_min=data_input_range_min,
                data_input_range_max=data_input_range_max,
                mlp_input_range_min=mlp_input_range_min,
                mlp_input_range_max=mlp_input_range_max,
                output_range_min=output_range_min,
                output_range_max=output_range_max,
                non_linearity=non_linearity,
                final_non_linearity=final_non_linearity,
                positional_encoding=positional_encoding,
                frequency_pos_encoding_freqs=frequency_pos_encoding_freqs
            ) for _ in range(n_heads)
        )
        
    def forward(self, x, additional_input=None):
        # Apply MLP
        res = torch.stack([self.mlps[i](x[i]) for i in range(len(self.mlps))], dim=0)
